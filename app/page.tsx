...
export default function DashboardPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (dashRes.status === 401) { setError("Session expired"); return }
      if (meRes.ok) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900 p-4">
        <div className="text-center space-y-4">
          ...
          <button onClick={() => window.location.href = "/"} className="text-sm text-blue-600 hover:underline">Go to dashboard</button>
        </div>
      </div>
    )
  }
      ...
      <AppShell userName={userName} onLogout={handleLogout}>
        <div className="space-y-6">
          <div>
            ...
            <div className="h-4 w-64 bg-slate-100 dark:bg-slate-800/50 rounded mt-2 animate-pulse" />
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="rounded-xl border border-slate-200 dark:border-slate-800 p-6">
                ...
                <div className="h-8 w-32 bg-slate-100 dark:bg-slate-800/50 rounded animate-pulse" />
              </div>
            ))}
          </div>
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 p-6">
              <div className="h-5 w-40 bg-slate-200 dark:bg-slate-800 rounded animate-pulse mb-4" />
              <div className="h-64 bg-slate-100 dark:bg-slate-800/50 rounded animate-pulse" />
            </div>
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 p-6">
              <div className="h-5 w-40 bg-slate-200 dark:bg-slate-800 rounded animate-pulse mb-4" />
              <div className="space-y-4">
                {[...Array(5)].map((_, i) => (
                  <div key={i} className="h-8 bg-slate-100 dark:bg-slate-800/50 rounded animate-pulse" />
                ))}
              </div>
            </div>
          </div>
